...
//import fr.ign.spark.io.DataTypeWithSize
...
def this(name: String, dataType: DataType) = { this(name,dataType match {
case ByteType => "uchar"
...
case class PlyHeader(
	...
	def this(
		...
		this("",littleEndian,0,schemas.map { case (name,(count,schema)) => PlyElement(name,littleEndian,count,schema.fields.map { f => new PlyProperty(f.name,f.dataType)})}.toSeq,obj_info,comments)
	}
